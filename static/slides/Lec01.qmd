...
You must sit the in class exam in order to receive points for the take home - in the event of a *documented* emergency the in class exam will be replaced by an in person oral exam (which must be scheduled within one week of the original exam date).
...
```{bash}
#| echo: false
uv python list | grep -v freethreaded | sed -e 's/ -> .*//' -e "s|$HOME|~|" \
  | awk '!seen[$1]++ {p=$2; for(i=3;i<=NF;i++) p=p" "$i; if (length(p)>36) p=substr(p,1,33)"..."; printf "%-40s %s\n", $1, p}' \
  | head -n 8
echo "..."
```
:::
...
```{bash}
#| echo: false
cd "${TMPDIR:-/tmp}" && rm -rf uv-demo && mkdir -p uv-demo && cd uv-demo
...
```{bash}
#| echo: false
cd "${TMPDIR:-/tmp}/uv-demo/my-project" && ls -la
```
:::
...
#| echo: false
#| class-output: toml
cd "${TMPDIR:-/tmp}/uv-demo/my-project" && cat pyproject.toml
```
:::
