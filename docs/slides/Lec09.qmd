...
::: {.aside}
If `null_values="NA"` is not supplied - columns with missing values will have a `str` dtype.
:::
...
::: {.aside}
The idiomatic way to subset is with expressions in `filter()` and `select()`
:::
...
::: {.aside}
Neither has an equivalent of `names_transform`, `year` is a string column until it is cast.
:::
...
::: {.aside}
`join()` method also exists, but it matches on the index rather than on columns - `merge()` is usually what you want.
:::
...
::: {.aside}
The same choice applies to filtering joins - `anti_join()` with the default `na_matches` will not report a missing key as unmatched if `y` also has one.
:::
