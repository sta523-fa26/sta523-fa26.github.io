...
A polars Series is a name, a dtype, and values - nothing else. With no index there is nothing to align on, so arithmetic is positional and lengths must match (or has length one).
...
`join()` also takes the type via `how` with the default of `"inner"`.
